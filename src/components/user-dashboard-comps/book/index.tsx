...
const BooksComp = () => {
    ...
    const retrieveGiftcards = () => {
        ...
        RETREIVE_BOOKS(query)
        .then((res: AxiosResponse<ApiResponse>) => {
            const { message, payload } = res.data;
            notify("success", message);
            setBooks(payload);
            dispatch(INITIALIZE_BOOKS(payload));
        })
        .catch((err: any) => {
    ...
    const sortData = (field: string) => {
        const sortedArray: any[] = sortArray(books, field);
    ...
    const handleSearchQuery = () => {
        setSearching(true);
        if(searchQuery !== '') {
            const filteredResults: Book[] = books.filter((item: Book) => Object.values(item).includes(searchQuery));
    ...
    return (
        <>
                ...
                <Card type='lg'>
                    ...
                    <div id="title">
                        <div className="flex flex-col sm:justify-between md:justify-between lg:flex-row lg:justify-between w-full">
                            <div className='mb-8'>
                                <h3 className='text-[#40b142] text-xl font-bold mb-1'>Books Records Table</h3>
                                <p className='text-[#7F7F80] text-sm'>Displaying {books.length} of {books.length} Giftcard Record(s)</p>
                            </div>
                            ...
                            <table className='table border w-full'>
                                <thead>
                                    <tr className='border-spacing-y-4'>
                                        <th className="text-left">Book code</th>
                                        <th>Name</th>
                                        <th>Author</th>
                                        <th>Status</th>
                                        <th>Date</th>
            ...
            <AppModalComp title=''>
                ...
                {
                    modalMode === 'view' && <BookDetailComp book={selectedBook} />
                }
                {
                    modalMode === 'update' && <BookUpdateForm book={selectedBook}  />
                }
                {
                    modalMode === 'delete' && <DeleteComp id={selectedBook?._id} action={handleDeleteRecord} deleting={deleting} />
                }
            </AppModalComp>
